...
const int PIN_RGB_AZUL = SHARED_D9;   // Pin para canal azul
...
void iniciarPractica4() {
  ...
  apagarTodasLasSalidas();
  pinMode(PIN_RGB_ROJO, OUTPUT);   // D9 - Canal Rojo
  pinMode(PIN_RGB_VERDE, OUTPUT);  // D10 - Canal Verde
  pinMode(PIN_RGB_AZUL, OUTPUT);   // D11 - Canal Azul
...
void mostrarInfoPractica4() {
  ...
  Serial.println(F(""));
  Serial.println(F("Configuración de Hardware:"));
  Serial.println(F("- LED RGB pin Rojo: D9 -> 220ohm -> Pin Rojo"));
  Serial.println(F("- LED RGB pin Verde: D10 -> 220ohm -> Pin Verde"));
  Serial.println(F("- LED RGB pin Azul: D11 -> 220ohm -> Pin Azul"));
...
void ejecutarPractica4() {
  ...
  analogWrite(PIN_RGB_ROJO, estadoP4.valorRojo);     // D9 -> Rojo
  analogWrite(PIN_RGB_VERDE, estadoP4.valorVerde);   // D10 -> Verde
  analogWrite(PIN_RGB_AZUL, estadoP4.valorAzul);     // D11 -> Azul
}
...
void probarColoresRGB() {
  ...
  Serial.println(F("- Pin D9 debe ir al canal ROJO del LED"));
  Serial.println(F("- Pin D10 debe ir al canal VERDE del LED"));
  Serial.println(F("- Pin D11 debe ir al canal AZUL del LED"));
}
